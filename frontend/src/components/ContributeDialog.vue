...
watch(
  ...
  (value) => {
    ...
    if (value) {
      ...
      contribution.value = JSON.parse(JSON.stringify(props.info?.contribution || { name: contributeStore.userInfo?.full_name || '', email: contributeStore.userInfo?.email || '', type: 'real' })) as Contribution;
      files.value = [];
