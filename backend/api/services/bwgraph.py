...
def bwgraph(bw: np.ndarray, 
           node_weights: Optional[np.ndarray] = None,
           connectivity: Optional[int] = None,
           interface_weight: float = 1.0) -> nx.Graph:
    """
    ...
    if dim == 2:
        ...
        source = np.column_stack((nonzero_indices[0], nonzero_indices[1], nonzero_indices[2]))
    ...
    if node_weights is None:
        ...
        weights_final = (node_weights.flat[source_final] + 
                        node_weights.flat[neighbors_final]) / 2
    
    # Adjust weights for interface edges
    for i in range(len(source_final)):
        if (_is_interface(bw, source_final[i], sz, dim) or 
            _is_interface(bw, neighbors_final[i], sz, dim)):
    ...
    edges = [(int(s), int(n), {'weight': float(w)}) 
...
if __name__ == "__main__":
    ...
    print(f"With interface weighting - Nodes: {G_2d_weighted.number_of_nodes()}, "
