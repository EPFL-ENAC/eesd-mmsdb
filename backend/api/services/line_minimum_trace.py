...
def calculate_line_minimum_trace(
    ...
    try:
        ...
        start_point = _find_nearest_mortar(image, start_point, pixel_height, pixel_length)
        end_point = _find_nearest_mortar(image, end_point, pixel_height, pixel_length)
        
        if start_point is None:
            return {
                "error": "No mortar found near start point",
                "success": False
            }
        
        if end_point is None:
            return {
                "error": "No mortar found near end point", 
                "success": False
            }
        ...
        if return_plot:
            plot_base64 = _generate_plot(image, pi, pj, start_point, end_point, LMT_type)
...
def _generate_plot(image, pi, pj, start_point, end_point, lmt_type):
    ...
    try:
        ...
        ax.plot(start_point[1], start_point[0], 'go', markersize=10, linewidth=2, label='Start')
        ax.plot(end_point[1], end_point[0], 'ro', markersize=10, linewidth=2, label='End')
...
def main():
    ...
    if result["success"]:
        ...
        print(f"Path Length: {len(result['path_coordinates']['pixel_coordinates'])} points")
